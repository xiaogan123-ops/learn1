...
public:
    ...
    void showLevelDialog(const QString &title, const QString &text1, const QString &text2);
    void setFocusToGame();
    ...
    settingWindow *settingwindow;
     ...
     void initLevelDialogs(); // 初始化对话内容
      CustomDialog *currentDialog = nullptr; // 对话框指针
       bool isDialogActive = false; // 对话框激活状态
      bool isbump=false;
       ...
       int m_stepSize;              // 当前移动步长
        bool gameFailed; // 游戏失败标志

       int fragmentsCollected;
       QMap<int, bool> fragmentCollectedInLevel;
       QPoint m_fragmentPos;
        bool specialEndingShown = false;

       QPoint m_woodenKitePos; // 木鸢传送阵位置
       QPoint m_transferTarget; // 传送目标位置
